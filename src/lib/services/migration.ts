...
/**
 * Runs a migration end to end: validate, import row by row, record every
 * outcome and finalize the migration record. Lives in the service layer so the
 * wizard, the employee importer and the automated tests all use one code path.
 */
export async function executeMigration(input: MigrationRunInput): Promise<MigrationRunResult> {
  const validation = await validateRows(input.dataType, input.rows, input.mapping, input.options);
  ...
  const migration = await queryOne<{ id: string }>(
    ...
    [`${input.sourceSystem} — ${dataTypeDefinition(input.dataType)?.label}`, input.sourceSystem, input.dataType,
      input.fileName, validation.total, JSON.stringify(input.mapping), batchId, input.actorId]);
  if (!migration) throw new Error("Could not start the migration.");

  let imported = 0, skipped = 0, failed = 0, warnings = 0;
  const records: Array<[number, Record<string, string>, string, string | null, string | null]> = [];

  for (const row of validation.rows) {
    if (row.status === "error") {
      failed += 1;
      records.push([row.rowNumber, row.values, "failed", row.messages.join(" · "), "Fix the source data and re-run"]);
      continue;
    }
    if (row.status === "duplicate" && input.options.skipDuplicates) {
      skipped += 1;
      records.push([row.rowNumber, row.values, "skipped", row.messages.join(" · "), "Duplicate skipped"]);
      continue;
    }
    try {
      const resolution = await importRow(input.dataType, row, input.options, input.sourceSystem, batchId, input.organizationId);
      imported += 1;
      if (row.status === "warning") warnings += 1;
      records.push([row.rowNumber, row.values, row.status === "warning" ? "warning" : "imported",
        row.messages.join(" · ") || null, resolution]);
    } catch (error) {
      failed += 1;
      records.push([row.rowNumber, row.values, "failed", (error as Error).message, "Import error"]);
    }
  }

  for (const [rowNumber, raw, status, message, resolution] of records) {
    await query(
      `insert into migration_records (migration_id, row_number, raw, status, message, entity_type, resolution)
       values ($1,$2,$3::jsonb,$4,$5,$6,$7)`,
      [migration.id, rowNumber, JSON.stringify(raw), status, message, input.dataType, resolution]);
  }
  ...
  await query(
    ...
    [migration.id, imported, skipped, failed, warnings,
      JSON.stringify({
        matchedOn: input.options.matchBy,
        createdEmployees: input.options.createMissingEmployees,
        createdCourses: input.options.createMissingCourses,
      })]);

  return { migrationId: migration.id, batchId, imported, skipped, failed, warnings, total: validation.total };
}
